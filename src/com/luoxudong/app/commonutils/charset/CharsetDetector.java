...
public class CharsetDetector {
	 ...
	 */
	public static String detectEncoding(URL testurl) {
	 ...
	 */
	public static String detectEncoding(File testfile) {
	 ...
	 * highest probability is returned.
	 */
	public static String detectEncoding(byte[] rawtext) {
		int[] scores;
	 ...
	 * encoding
	 */
	static int gb2312_probability(byte[] rawtext) {
		int i, rawtextlen = 0;
	 ...
	 * encoding
	 */
	static int gbk_probability(byte[] rawtext) {
		int i, rawtextlen = 0;
		int dbchars = 1, gbchars = 1;
		long gbfreq = 0, totalfreq = 1;
		float rangeval = 0, freqval = 0;
		...
		rangeval = 50 * ((float) gbchars / (float) dbchars);
		freqval = 50 * ((float) gbfreq / (float) totalfreq);
		// For regular GB files, this would give the same score, so I
		// handicap
		// it slightly
	 ...
	 */
	static int hz_probability(byte[] rawtext) {
		int i, rawtextlen;
		...
		if (hzstart > 4) {
			...
			rangeval = 41;
		} else if (hzstart > 0) { // Only 39 in case the sequence happened
									// to
									// occur
	 ...
	 */
	static int big5_probability(byte[] rawtext) {
		// int score = 0;
		int i, rawtextlen = 0;
	 ...
	 * encoding
	 */
	static int euc_tw_probability(byte[] rawtext) {
		int i, rawtextlen = 0;
	 ...
	 */
	static int iso_2022_cn_probability(byte[] rawtext) {
		int i, rawtextlen = 0;
		...
		for (i = 0; i < rawtextlen - 1; i++) {
			if (rawtext[i] == (byte) 0x1B && i + 3 < rawtextlen) { // Escape
				...
				if (rawtext[i + 1] == (byte) 0x24 && rawtext[i + 2] == 0x29
						&& rawtext[i + 3] == (byte) 0x41) { // GB Escape $ )
															// A
					i += 4;
				...
				if (rawtext[i] == (byte) 0x1B && i + 2 < rawtextlen
						...
						&& rawtext[i + 2] == (byte) 0x42) { // ASCII: ESC (
															// B
					i += 2;
				}
			}
	 ...
	 */
	static int utf16_probability(byte[] rawtext) {
		// int score = 0;
		// int i, rawtextlen = 0;
		// int goodbytes = 0, asciibytes = 0;
	 ...
	 * reduced
	 */
	static int ascii_probability(byte[] rawtext) {
		int score = 70;
		int i, rawtextlen;
		rawtextlen = rawtext.length;
		for (i = 0; i < rawtextlen; i++) {
			if (rawtext[i] < 0) {
				score = score - 5;
			} else if (rawtext[i] == (byte) 0x1B) { // ESC (used by ISO
													// 2022)
				score = score - 5;
			}
		}
